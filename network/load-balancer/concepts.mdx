...
Each Load Balancer has one or several backends. A backend is a configuration used by the Load Balancer to forward traffic to its destination servers. Each backend uses the specified configuration (port, protocol, proxy protocol etc) to forward traffic to its designated [backend servers](#backend-servers). For more information about configuring your Load Balancer's backend(s), see our [dedicated documentation](/network/load-balancer/reference-content/configuring-load-balancer#configuring-a-backend).
...
An certificate (aka SSL certificate / TLS certificate / TLS/SSL certificate) is a digital certificate that enables an encrypted connection between a client and a web server. 

<Message type="tip">

  You may hear certificates referred to as "SSL certificates", "TLS certificates" or "TLS/SSL certificates". These are all the same thing. SSL was the protocol initially used for encryption, though modern certificates in fact use TLS (Transport Layer Security) as the encryption protocol.

</Message>

You can [add a  certificate](/network/load-balancer/how-to/add-certificate) to your Load Balancer's frontend to enable the Load Balancer to encrypt/decrypt incoming traffic, necessary for [SSL bridging and SSL offloading configurations](/network/load-balancer/reference-content/configuring-load-balancer). Scaleway Load Balancer enables you to generate a Let's Encrypt certificate, or import a third-party or self-signed certificate.
...
[SSL bridging](/network/load-balancer/reference-content/configuring-load-balancer#configuring-a-load-balancer-for-ssl-bridging) removes SSL-based encryption from HTTPS traffic coming into the Load Balancer (as with SSL offloading), but then initiates a new SSL connection to re-encrypt traffic between the Load Balancer and the backend servers.
...
[SSL offloading](/network/load-balancer/reference-content/configuring-load-balancer#configuring-a-load-balancer-for-ssl-offloading) removes SSL-based encryption from HTTPS traffic coming into the Load Balancer.
