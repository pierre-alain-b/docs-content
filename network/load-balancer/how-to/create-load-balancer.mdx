...
8. Configure your Load Balancer's **[backend](/network/load-balancer/concepts/#backends)** by entering a **name**, and the **protocol** and **port** it should use to connect to the backend servers. Select also a **proxy** protocol option, **health check** method and add the **server IPs** (one or many) of the backend server(s) it should direct traffic to. 
