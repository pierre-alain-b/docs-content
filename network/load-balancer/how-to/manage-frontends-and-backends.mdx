...
5. Enter a **name**, **port** and (if necessary) [**SSL certificate(s)**](/network/load-balancer/how-to/add-certificate) for your new frontend. Note that each frontend must listen on a different port. Note that each frontend must listen on a different port. For help configuring your frontend, see our [dedicated reference documentation](/network/load-balancer/reference-content/configuring-load-balancer).
6. Choose between **using an existing backend** or **creating a new backend**.
    ...
    - If creating a new backend, click **Create a new Backend** and enter a name, protocol, port proxy, health check type, server IP(s) and advanced settings as required. For help configuring your frontend, see our [dedicated reference documentation](/network/load-balancer/reference-content/configuring-load-balancer)
...
4. Click the green **+** button.
    <Lightbox src="scaleway-add-backends.webp" />
5. Enter a name, protocol, port, proxy, health check type,  server IP(s) and advanced settings as required for your new backend. For help configuring your frontend, see our [dedicated reference documentation](/network/load-balancer/reference-content/configuring-load-balancer)
