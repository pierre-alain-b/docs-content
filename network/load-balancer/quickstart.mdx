...
8. Configure your Load Balancer's **[backend](/network/load-balancer/concepts/#backends)** by entering a **name**, and the **protocol** and **port** it should use to connect to the backend servers. Enter also a **proxy**, **health check** method and the **server IPs** of the backend servers it should direct traffic to. You can [add additional backends](#how-to-add-backends) after creating your Load Balancer, if required.
...
4. Click the green **+** button to launch the frontend creation wizard.
...
6. Choose between **using an existing backend** or **creating a new backend**. Your frontend will direct traffic to the backend you choose here.
    ...
    - If creating a new backend, click **Create a new backend** and enter a name, protocol, port proxy, health check type, server IP(s) and advanced settings as required. See our [dedicated documentation](/network/load-balancer/reference-content/configuring-load-balancer/) for help configuring your backend
7. Click **Create Frontend**.

## How to add backends

You can add more backends to your Load Balancer after its creation. Each backend can be connected to a different frontend and different backend servers as required, allowing you to route traffic through your Load Balancer according to your needs.
...
5. Enter a name, protocol, port, proxy, health check type,  server IP(s) and advanced settings as required for your new backend. See our [dedicated page](/network/load-balancer/reference-content/configuring-load-balancer/) for help configuring your backend, including details of how to set up SSL bridging, offloading or passthrough.
